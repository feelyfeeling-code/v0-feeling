...
const TEST_TYPES: { value: string; label: string }[] = [
  ...
  { value: 'process_com', label: 'Process Com' },
  { value: 'other', label: 'Autre' },
]

const INTEGRATED_TEST_QUESTIONS: { id: string; text: string }[] = [
  { id: 'q1', text: 'Je suis à l\'aise dans des situations sociales nouvelles.' },
  { id: 'q2', text: 'Je préfère planifier plutôt qu\'improviser.' },
  { id: 'q3', text: 'Je prends des décisions rapidement même dans l\'incertitude.' },
  { id: 'q4', text: 'Je préfère le travail en équipe au travail solo.' },
  { id: 'q5', text: 'Je suis curieux(se) d\'apprendre en dehors de mon domaine.' },
  { id: 'q6', text: 'Je reste calme face au stress.' },
  { id: 'q7', text: 'J\'exprime mon désaccord sans hésiter.' },
  { id: 'q8', text: 'Je préfère suivre les règles plutôt que les remettre en question.' },
  { id: 'q9', text: 'J\'aime résoudre des problèmes complexes.' },
...
const WHEN_IN_ELEMENT_OPTIONS = [
  { value: 'resoudre_probleme', label: 'Je résous un problème complexe' },
  { value: 'collaborer', label: 'Je collabore avec mon équipe' },
  { value: 'creer', label: 'Je crée quelque chose de nouveau' },
  { value: 'impact_concret', label: 'Je vois l\'impact concret de mon travail' },
  { value: 'organisation', label: 'Je suis bien organisé(e) et efficace' },
]

const REACTION_FAILURE_OPTIONS = [
  { value: 'rebondit_vite', label: 'Je rebondis vite' },
  { value: 'analyse', label: 'J\'analyse ce qui s\'est passé' },
  { value: 'besoin_temps', label: 'J\'ai besoin de temps' },
  { value: 'cherche_soutien', label: 'Je cherche du soutien' },
]

const APPROACH_PROBLEM_OPTIONS = [
  { value: 'cherche_methode', label: 'Je cherche une méthode' },
  { value: 'teste_solutions', label: 'Je teste des solutions' },
  { value: 'demande_aide', label: 'Je demande de l\'aide' },
  { value: 'analyse_avant', label: 'J\'analyse avant d\'agir' },
]

const MAX_TRAITS = 3

// Ordre des sous-étapes : test-question → (test-intro → test-questions si Non) → traits → behavior
type SubStep = 'test-question' | 'test-intro' | 'test-questions' | 'traits' | 'behavior'

function getTraitLabel(value: string): string {
  return AVAILABLE_TRAITS.find((t) => t.value === value)?.label ?? value
}

function firstUnansweredIndex(answers: Record<string, number> | undefined): number {
  const safe = answers ?? {}
  const idx = INTEGRATED_TEST_QUESTIONS.findIndex((q) => safe[q.id] === undefined)
...
function initialSubStep(data: OnboardingData['personality']): SubStep {
  ...
  if (answeredCount > 0 && !data.has_taken_test && (data.traits ?? []).length < MAX_TRAITS) return 'test-questions'
  if ((data.traits ?? []).length === MAX_TRAITS && !data.when_in_element) return 'behavior'
  if ((data.traits ?? []).length > 0 && (data.traits ?? []).length < MAX_TRAITS) return 'traits'
  return 'test-question'
}

export function PersonalityStep({ data, onUpdate, onNext, onPrev }: PersonalityStepProps) {
  ...
  const toggleTrait = (value: string) => {
    ...
    if (traitsList.length >= MAX_TRAITS) {
      toast.error(`Tu as atteint le maximum de ${MAX_TRAITS}. Retire-en un pour en choisir un autre.`)
      return
    }
  ...
  const answeredCount = useMemo(() => Object.keys(testAnswers).length, [testAnswers])
  ...
  const currentAnswer = currentQuestion !== undefined ? testAnswers[currentQuestion.id] : undefined

  const selectAnswer = (value: number) => {
    onUpdate({ test_answers: { ...testAnswers, [currentQuestion.id]: value } })
  }
  ...
  const canContinueBehavior =
    !!data.when_in_element &&
    !!data.reaction_to_failure &&
    !!data.approach_complex_problem

  // -------------------- Navigation --------------------
  const handlePrev = () => {
    switch (subStep) {
      case 'test-question': onPrev(); break
      case 'test-intro': setSubStep('test-question'); break
      case 'test-questions':
        if (questionIndex > 0) setQuestionIndex(questionIndex - 1)
        else setSubStep('test-intro')
        break
      case 'traits':
        if (hasTakenTest) setSubStep('test-question')
        else setSubStep('test-questions')
        break
      case 'behavior': setSubStep('traits'); break
    }
  }
  ...
  const handleNext = () => {
    switch (subStep) {
      case 'test-question':
        if (hasTakenTest) setSubStep('traits')
        else setSubStep('test-intro')
        break
      case 'test-intro':
      ...
      case 'test-questions':
        ...
        else setSubStep('traits')
        break
      case 'traits': setSubStep('behavior'); break
      case 'behavior': onNext(); break
    }
  }

  const isDisabled =
    (subStep === 'test-question' && !canContinueTestQuestion) ||
    (subStep === 'test-questions' && currentAnswer === undefined) ||
    (subStep === 'traits' && !canContinueTraits) ||
    (subStep === 'behavior' && !canContinueBehavior)
  ...
  return (
    <div className="max-w-xl mx-auto py-8 relative">
      ...
      <div className="text-center mb-8">
        ...
        {subStep === 'test-question' && (
          <h1 className="text-2xl md:text-3xl font-bold">
            As-tu déjà passé un test<br />de personnalité&nbsp;?
          </h1>
        )}
        {subStep === 'test-intro' && (
          <h1 className="text-2xl md:text-3xl font-bold">
            Mini-test de<br />personnalité
          </h1>
        )}
        ...
        {subStep === 'traits' && (
          <>
            <h1 className="text-2xl md:text-3xl font-bold">
              Choisis tes 3 traits de<br />personnalité dominants
            </h1>
        ...
        {subStep === 'behavior' && (
          <h1 className="text-2xl md:text-3xl font-bold">
            Ton comportement<br />au travail
          </h1>
        )}
      </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className={`p-6 rounded-2xl border-2 font-semibold transition-all ${
                hasTakenTest ? 'border-primary bg-primary/20' : 'border-border hover:border-primary/50'
              }`}
            >
              Oui
            ...
            <button
              type="button"
              onClick={() => onUpdate({ has_taken_test: false, test_type: '', test_result: '' })}
              aria-pressed={!hasTakenTest}
              className={`p-6 rounded-2xl border-2 font-semibold transition-all ${
                !hasTakenTest ? 'border-primary bg-primary/20' : 'border-border hover:border-primary/50'
              }`}
          ...
          {hasTakenTest && (
            <div className="space-y-4 pt-2">
              <div className="space-y-2">
                <Label htmlFor="test-type">Si oui, lequel&nbsp;?</Label>
                <Select value={testType} onValueChange={(value) => onUpdate({ test_type: value })}>
              ...
              <div className="space-y-2">
                <Label htmlFor="test-result">Quel était ton résultat&nbsp;?</Label>
                <Input
                  id="test-result"
      ...
      {subStep === 'test-intro' && (
        <div className="space-y-6">
          <div className="bg-muted/50 rounded-2xl p-6 space-y-4">
            <p className="text-sm">
              Ce mini-test va nous aider à mieux cerner comment tu fonctionnes au travail :
              ta manière de prendre des décisions, de collaborer, de gérer le stress et d&apos;aborder la nouveauté.
            </p>
            <p className="text-sm">
              Il comporte <strong>{INTEGRATED_TEST_QUESTIONS.length} questions</strong> simples.
        ...
        <>
          <div className="mb-4 flex items-center justify-between">
            <span className="text-sm font-medium text-muted-foreground">
              {traitsList.length}/{MAX_TRAITS} sélectionné{traitsList.length > 1 ? 's' : ''}
            </span>
          </div>
              ...
              <div className="space-y-2">
                {traitsList.map((value, index) => (
                  ...
                  >
                    ...
                    <div className="flex-1 text-sm font-semibold">{getTraitLabel(value)}</div>
      ...
      {/* -------- 4d — Comportement au travail -------- */}
      {subStep === 'behavior' && (
        <div className="space-y-8">
          {/* Quand dans son élément */}
          <div className="space-y-3">
            <p className="text-sm font-semibold">
              Quand tu te sens vraiment dans ton élément, c&apos;est quand...
            </p>
            <div className="space-y-2">
              {WHEN_IN_ELEMENT_OPTIONS.map((opt) => {
                const selected = data.when_in_element === opt.value
                return (
                  <button
                    key={opt.value}
                    type="button"
                    onClick={() => onUpdate({ when_in_element: opt.value })}
          ...
          <div className="space-y-3">
            <p className="text-sm font-semibold">
              Comment tu réagis face à un échec&nbsp;?
            </p>
            <div className="grid grid-cols-2 gap-2">
              {REACTION_FAILURE_OPTIONS.map((opt) => {
                const selected = data.reaction_to_failure === opt.value
                return (
                  <button
                    key={opt.value}
                    type="button"
                    onClick={() => onUpdate({ reaction_to_failure: opt.value })}
                    aria-pressed={selected}
          ...
          <div className="space-y-3">
            <p className="text-sm font-semibold">
              Face à un problème complexe, tu es plutôt...
            </p>
            <div className="grid grid-cols-2 gap-2">
              {APPROACH_PROBLEM_OPTIONS.map((opt) => {
                const selected = data.approach_complex_problem === opt.value
                return (
                  <button
                    key={opt.value}
                    type="button"
                    onClick={() => onUpdate({ approach_complex_problem: opt.value })}
                    aria-pressed={selected}
                    className={`p-3 rounded-2xl border-2 text-sm font-medium text-center transition-all ${
                      selected ? 'border-primary bg-primary/20' : 'border-border hover:border-primary/50'
                    }`}
